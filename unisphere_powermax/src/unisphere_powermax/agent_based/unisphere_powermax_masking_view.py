...
def check_masking_view_volume_summary(item, params, section):
    ...
    warn, crit = params['levels'][1]
    info_text = "volumes online: %s%% %s/%s (warn/crit <%s%%/<%s%%)" % ((p_online, n_ready_volumes, n_volumes) + params['levels'][1])
    if p_online < crit:
        state = State.CRIT
    elif p_online < warn:
        state = State.WARN
    perfdata = [('percent_volumes_online', "{}%".format(p_online), warn, crit),
                ('absolute_volumes_online', "{}%".format(n_ready_volumes), float(warn)/100.0*n_volumes, float(crit)/100.0*n_volumes)]
