...
snmp_section_cisco_vpc_host_link = SimpleSNMPSection(
...
def check_cisco_vpc_host_link(section_cisco_vpc_host_link, section_if64):
    ...
    if all(entry["consistency_status"] == "1" for entry in section_cisco_vpc_host_link.values()):
...
check_plugin_cisco_vpc_host_link = CheckPlugin(
    name="cisco_vpc_host_link",
